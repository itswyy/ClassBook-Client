...
- (void)menuItemTapped:(QuadCurveMenuItem *)item {
    ...
    //modify by itswyy menuItem点击后视图消失
    [self performSelector:@selector(dismissMenuView) withObject:nil afterDelay:0.2f];
}
...
- (void)performCloseMenu {
    ...
    //modify by itswyy
    //等待收回动画完毕后再消失视图，否则会闪没
    [self performSelector:@selector(dismissMenuView) withObject:nil afterDelay:0.5];
