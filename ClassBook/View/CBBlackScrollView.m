...
@implementation CBBlackScrollView
@synthesize delegateForBlackboard;
...
-(void)selectALayerToShow:(UITapGestureRecognizer *)recognizer{
    ...
    for (CALayer *layer in layersArray) {
        if (CGRectContainsPoint(layer.frame, location)) {
            ...
            [delegateForBlackboard flipScrollView:self didSelectAtIndex:index withLayer:layer];
            break;
        }
        index++;
    }
...
-(void)loadViewData
{
    ...
    for (int i = 0; i < numberOfItems; i++) {
        CBInScrollViewLayer *layer = [CBInScrollViewLayer layer];
        ...
        textLayer.string = [[dataMutableArray objectAtIndex:i] objectForKey:@"date"];
...
- (void)layoutSubviews
{
    ...
    for (CBInScrollViewLayer *aLayer in layersArray) {
...
-(void)resetSelection{
    if (layersArray.count > selectIndex) {
        CBInScrollViewLayer *aLayer = [layersArray objectAtIndex:selectIndex];
        NSUInteger k = 3;
